...
8. If the user asks to certify or promote PASS-SCOPED to PASS-TRACKED, require promotion certificate v2 with exact-string `promotion_schema_version: "2.0"`, exact required top-level JSON types, and the fixed nine-role `promotion-evidence-v2` canonical DAG; fresh deterministic suites; fresh allowlisted official validators; live Claude Code fixtures; formal result `2.0` with trace authentication, a standalone endpoint-checked target copy, prompt companion, and target-copy companion; and explicit `downstream_review`. The copy is permission-hardened, but `temporal_immutability_enforced` is `false`, so an otherwise `PASS-TRACKED` formal result is capped at `PASS-SCOPED`. Runtime execution additionally requires supported Linux child-subreaper plus process-group containment before `Popen`; unavailable containment or incomplete cleanup fails closed. Every long-running release CLI acquires caller-controlled output parents component-by-component with `O_DIRECTORY`/`O_NOFOLLOW` before work and retains them through descriptor-relative installation; role/alias decisions are frozen against held identities, and observed parent substitution fails closed without redirected writes. This is capability-bound output handling, not temporal isolation. Formal children receive only the runner-owned `/proc/<runner-pid>/fd/N` path through `pass_fds`, with procfs-visible `Pid:`/direct-parent `PPid:` authentication that resists child-FD close/rebind. Text validator captures never authorize. Absent official tools scope fresh execution only through the explicit flag while both official-policy nodes and the fixed dependencies remain mandatory; installed failures fail. In v1.0.3, `certify_pass_tracked_upgrade.py` also caps a complete modeled result at `PASS-SCOPED` / `CAPPED`, `promotion_authorized: false`, `satisfied_profile: promotion-contract-v2-complete`, both explicit unresolved Issue #5 obligations, and a nonzero exit. Generic `ntt_gate.py` PASS-TRACKED semantics remain unchanged.
   Promotion v2 also requires a declared `origin` cross-bound to live provenance schema `2.0` and formal `evidence_origin`, and reconstructs `promotion-method-m-v2`. Its claims carry nonempty `scope`, claim-contract schema `1.1`, and complete schema-`1.1` modal world contracts. A terminal Claude SDK ResultMessage authorizes only through canonical nonempty `result`; `api_error_status`, `deferred_tool_use`, non-structured `structured_output`, `terminal_reason`, and `stop_reason` remain explicit fail-closed controls.
...
- Claim-contract schema `1.1` binds the complete claim plus certificate assurance, including the full downstream-policy object and claim inventory. Modal authorization uses closed schema-`1.1` operator/target/outcome fields; `outcome` and `observed_outcome` remain separately hashed, while prose is bound explanation rather than a polarity oracle. When validating this package with `--downstream-policy package-self`, its certificate-wide modal ID multiset must equal all 80 immutable reviewed transitions exactly once with pinned claim/kind assignments.
...
10. Formal result schema `2.0`, which binds the standalone endpoint-checked target copy, exact companion manifest, package-tree identity, run ID, target pre/post endpoint identity, matching schema-`2.0` `evidence_origin`, `temporal_immutability_enforced: false`, and verified `linux-child-subreaper-plus-process-group` containment. Trace authentication requires one final canonical successful ResultMessage after every lane result and checks `api_error_status`, `deferred_tool_use`, `structured_output`, `terminal_reason`, and `stop_reason` explicitly. Its outputs retain no-follow parent capabilities from before evaluation through descriptor-relative exclusive creation, link/rename installation, and directory fsync. Children use only the runner-owned `/proc/<runner-pid>/fd/N` capability through `pass_fds`; the procfs-visible runner `Pid:` must equal the child's direct-parent `PPid:`, so child-FD close/rebind and self/unrelated/noncanonical/closed/file forms fail. Promotion selects the result only through the typed `formal.result` role; no basename substitution or glob-first selection is allowed, while unrelated nonreserved files may remain. An otherwise `PASS-TRACKED` formal result is capped at `PASS-SCOPED` while temporal immutability remains unenforced; unavailable process containment refuses execution, and unavailable procfd capability returns `INVALID_INPUT` before requested output mutation.
...
The synthetic promotion aggregate is `python3 skills/nozickian-verify/scripts/run_promotion_certifier_contract_tests.py .`. Its expected `46/46` result invokes the production certifier CLI for the baseline and every negative case, including formal coordinator result/argv identity substitutions, but it is contract evidence rather than real runtime authentication. Promotion requires at least one well-formed claim and actual canonical strict-gate evaluation with a nonempty all-passing result set; malformed or empty claims produce canonical `FAIL` plus `failure_kind`, never an unhandled traceback. Official and formal stream decisions bind complete captured bytes rather than bounded excerpts or tails. Gate Markdown, certifier JSON/Markdown, formal output/compatibility JSON, live transcript/JSON, validator Markdown, deterministic wrapper JSON, and fixed-manifest outputs use pre-acquired held no-follow parents and descriptor-relative installation. Alias classification is frozen, ancestor replacement cannot redirect writes, and observed identity/ancestry drift fails closed; these checks are not temporal isolation. Formal children use only an authenticated runner-PID-qualified procfd capability, and missing procfd support fails before requested output creation. An existing private regular `--json` file is intentionally replaced atomically, gate Markdown requires a fresh final name, and validator Markdown must remain external to the package tree.
...
The maintainer's actual `validate_package.py . --self-test` invocation is bound from its start through completion by one bounded no-follow package-entry snapshot containing captured regular-file bytes. Authorization reads and all self-test fixture trees consume or materialize only those captured bytes, not a later lexical source or mirror path. Independent path/metadata and aggregate-file budgets apply, while Git discovery/index capture has separate time, combined-output, and entry ceilings. Finalization independently re-snapshots both the held source and the private materialized mirror. The non-recursive stable-tree check then runs the reviewed deterministic CLI entry points twice as subprocesses with unique external outputs. Optional validators, live runtime evaluation, and caller-supplied promotion evidence remain explicit scope exclusions; they are not relabeled as deterministic equivalents. A self-test `--markdown` destination must resolve outside the package root and is written by atomic replacement.
