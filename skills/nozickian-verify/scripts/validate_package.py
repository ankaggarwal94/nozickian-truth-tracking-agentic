...
def git_tracked_files(root: Path) -> List[str] | None:
    """Return git-tracked paths (repo-relative POSIX) under root, or None.

    Returns None when root is not inside a git work tree (e.g. the mutation and
    benign-variation temp copies produced by shutil.copytree, which carry no
    .git). Callers treat None as "not a git work tree; tracked-cruft check not
    applicable" so the rule fires only on real checkouts.
    """
    try:
        proc = subprocess.run(
        ...
        if listed.returncode != 0:
            return None
        return [x for x in listed.stdout.split("\0") if x]
...
# fixed-token STALE_PROVENANCE_PATTERNS grep (scan_release_provenance_hygiene)
...
# are the retained, precise defenses.
...
class Validator:
    ...
    def check_closed_surface(self) -> None:
        # No git-TRACKED build cruft. Inventory scans intentionally SKIP untracked
        # cruft (correct for dev checkouts with local __pycache__/.pyc/.DS_Store),
        # but a cruft path that is committed to git ships in the package and must
        # fail CRITICAL. Not applicable to the mutation/benign temp copies, which
        # carry no .git (git_tracked_files returns None there).
        tracked = git_tracked_files(self.root)
        if tracked is None:
            self.add("no git-tracked build cruft (__pycache__/.pyc/.DS_Store)", True, details="not a git work tree; tracked-cruft check not applicable")
        else:
            tracked_cruft = sorted(rel for rel in tracked if _is_cruft_relpath(rel))
            self.add("no git-tracked build cruft (__pycache__/.pyc/.DS_Store)", not tracked_cruft, details=", ".join(tracked_cruft[:20]))
