...
For v1.0.3, this audit models the complete promotion contract but does not authorize promotion. Even when every modeled check passes, `certify_pass_tracked_upgrade.py` returns `status: PASS-SCOPED`, `outcome: CAPPED`, `promotion_authorized: false`, `satisfied_profile: promotion-contract-v2-complete`, both unresolved Issue #5 obligations, and a nonzero exit. Generic `ntt_gate.py` retains its ordinary `PASS-TRACKED` semantics. The formal runner independently caps an otherwise `PASS-TRACKED` result at `PASS-SCOPED` because permission-hardened endpoint copies are not temporally immutable to a same-UID actor. Process execution requires supported Linux subreaper plus process-group containment and otherwise fails before `Popen`.
...
4. Formal result `2.0` executed through Claude Code against the standalone endpoint-checked target copy, with exact package/run/target endpoint identities and every typed companion self-contained under its bundle result directory; the result truthfully records temporal immutability as unenforced and is capped at `PASS-SCOPED`, while mandatory Linux subreaper/process-group containment is established before execution and completes without survivors.
...
Gate Markdown, certifier `--json`/`--markdown`, formal `--output-dir` plus compatibility `--json`, live transcript plus optional JSON, validator Markdown, every deterministic contract/regression `--json` wrapper, and the fixed behavior/stable-release manifests open their parent component-by-component with `O_DIRECTORY`/`O_NOFOLLOW` before long-running work and hold it across every write. Exclusive temporaries or new files are installed through descriptor-relative link/rename as applicable and followed by directory fsync. Role/alias classification is frozen from lexical names plus held identities; formal canonical-result classification is not recomputed after writing, certifier destinations cannot alias, and live JSON cannot alias a selected transcript. Direct/ancestor links, special or hardlink sentinels, and ancestor-to-symlink or real-directory substitution fail with bounded structured output and no redirected overwrite. Validator Markdown also verifies that the held parent remains outside the package tree. Endpoint and capability checks are not temporal isolation; they do not exclude every same-UID mutation between observations.

Formal children receive only an authenticated runner-owned `/proc/<runner-pid>/fd/N` capability; missing support is `INVALID_INPUT` before requested output mutation. Each output keeps its declared fresh-versus-replaceable final-name policy. An explicitly supplied existing private regular `--json` file remains intentionally replaceable, gate Markdown requires a fresh name, validator Markdown requires an external parent, and an existing regular file is never accepted as `--output-dir`; that path must be a real directory or a safely created new directory.
