...
The process exits nonzero. The modeled-promotion cap is certifier-specific; generic `ntt_gate.py` `PASS-TRACKED` semantics remain unchanged. The formal runner separately applies its endpoint/containment cap. The 43-case aggregate invokes the production certifier CLI for its complete baseline and every negative, but remains synthetic contract evidence rather than runtime authentication.
