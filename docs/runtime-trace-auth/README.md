...
Review `self_validation/formal_runner_contract_results.json` for cases covering fake traces, nested event smuggling, role inversion, duplicate tool-use IDs, missing results, generic results, text-only masquerades, unexpected agent calls, an early general-purpose event followed by a large valid-looking tail, complete transcript binding, and unsafe output destinations.
